...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
   ...
   <a href="${pageContext.request.contextPath}/studentcourse/list/${user.id}">View Course Enrolled</a>
   ...
   <%-- <table>
     ...
     <c:forEach items="${ViewGrade}" var="user" varStatus="index">
       ...
       <td>${user.studentEnrolId}</td>
         ...
         <td>${user.course.credit }</td>
      <td>${user.score}</td> 

<td><c:choose>
				<c:when test ='${user.score>80}'>A</c:when>
				<c:when test ='${user.score>70}'>B</c:when>
				<c:when test ='${user.score>60}'>C</c:when>
				<c:when test ='${user.score>50}'>D</c:when>
				<c:when test ='${user.score<50}'>F</c:when>
			</c:choose></td>
